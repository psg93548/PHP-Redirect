...
// Gère les 3 cas de réponse possibles de l'API GG :
//   1. URL brute   -> redirection serveur classique (header Location)
//   2. Snippet JS  -> injecté dans le <head> de la page (cloaking JS-side)
//   3. Vide        -> visiteur non ciblé / GEO non couvert -> page normale
// ============================================================
$ggRaw = file_get_contents(
...
$ggRaw     = trim((string) $ggRaw);
$ggMode    = 'empty';   // empty | url | snippet
$ggSnippet = '';

if ($ggRaw !== '') {
    if (preg_match('#^https?://#i', $ggRaw)) {
        // Cas 1 : URL brute renvoyée -> on redirige côté serveur, on arrête tout de suite
        $ggMode = 'url';
        header("Location: " . $ggRaw);
        exit();
    } else {
        // Cas 2 : snippet JS/HTML renvoyé -> on l'injecte dans le <head> plus bas
        $ggMode    = 'snippet';
        $ggSnippet = $ggRaw;
    }
}
// Cas 3 ($ggMode === 'empty') : on laisse filer, page affichée normalement
?>
<!DOCTYPE html>
<html lang="fr">
<head>
  ...
  <?php
    // Injection du snippet GG (cas "snippet") directement dans le <head>
    if ($ggMode === 'snippet') {
        echo $ggSnippet;
    }
  ?>
...
<div class="debug-bar">
  ...
  <?php
    ...
    $modeLabel = array(
        'empty'   => '⚪ Vide (aucune offre / visiteur non ciblé)',
        'url'     => '✅ URL brute (redirection serveur déclenchée)',
        'snippet' => '🟡 Snippet JS reçu (injecté dans &lt;head&gt;)',
    );
    echo " &nbsp;|&nbsp; Résultat GG : <b>" . $modeLabel[$ggMode] . "</b>";

    if ($ggMode === 'snippet') {
        echo " &nbsp;|&nbsp; Taille snippet : <b>" . strlen($ggSnippet) . " octets</b>";
    }
  ?>
</div>
...
<div class="test-label">🧪 TEST · PHP Redirect (mode: <?php echo $ggMode; ?>)</div>
